...
class TelemetryValidator
{
    ...
    /**
     * Validates all stored telemetry data.
     * This differs to the above function, in that the fields passed in here are more extrapolated and less structured.
     *
     * @param array $tainted_telemetry_data
     * @return array
     */
    public function validateStoredTelemetryData(array $tainted_telemetry_data) : array
    {
        $sanitised_telemetry_data = array();
        ...
        foreach($tainted_telemetry_data as $telemetry_message) {
            ...
            foreach($telemetry_message as $element_key => $element_value) {
                ...
                switch($element_key) {
                    ...
                    case 'switch_three':
                    case 'switch_four':
                        $filtered_telemetry_data[$element_key] = $this->validateSwitch($element_value);
                        break;
                    case 'fan':
                        $filtered_telemetry_data[$element_key] = $this->validateFanState($element_value);
                        break;
                    ...
                    case 'keypad':
                        ...
                        break;
                    case 'timestamp':
                        $filtered_telemetry_data[$element_key] = $this->validateDate($element_value);
                        break;
                    default:
                        break;
                }
    ...
    /**
     * Validates the timestamp generated by MySQL.
     *
     * @param string $tainted_date
     * @return string
     */
    private function validateDate(string $tainted_date) : string
    {
        $cleaned_date = '';

        if (!empty($tainted_date)) {
            $cleaned_date = strtotime($tainted_date) ? date('Y-m-d H:i:s', strtotime($tainted_date)) : '';
        }
    ...
    /**
     * Validates an individual switch value.
     *
     * @param string $tainted_switch
     * @return int
     */
    private function validateSwitch(string $tainted_switch) : int {
        $cleaned_switch = 0;

        $switch_validation_options = array(
            'options' => array(
                'min_range' => 0,
                'max_range' => 1,
                'default' => 0
        ...
        if (!empty($tainted_switch)) {
            $cleaned_switch = filter_var($tainted_switch, FILTER_VALIDATE_INT, $switch_validation_options);
        }

        return $cleaned_switch;
    }
